...
public class InworldNDKClient : MonoBehaviour, IInworldClient
{
...
VSAttribution.VSAttribution.SendAttributionEvent("Login Runtime", InworldAI.k_CompanyName, InworldAI.User.Account);
    ...
    {
        Debug.Log("RECEIVED A NDK PACKET " + packet.GetType().Name);
        ...
        else if (packet.Text != null)
        {
            ...
            TextEvent te = new TextEvent(packet);
        ...
        else if (packet.AudioChunk != null)
        {
            ...
            {
                Debug.Log("packet.Timestamp is null");
            }

            if (packet.Routing == null)
            {
                Debug.Log("packet.Routing is null");
            }

            if (packet.PacketId == null)
            {
                Debug.Log("packet.PacketId is null");
            }
            ...
            else
            {
                ...
                if (packet.DataChunk.AdditionalPhonemeInfo == null)
                {
                    Debug.Log("packet.DataChunk.AdditionalPhonemeInfo is null");
                }
                else
                    phonemesValid = true;
            }
            //phonemesValid = true;
            AudioChunk chunk = phonemesValid? new AudioChunk(packet) : new AudioChunk(packet.AudioChunk.Chunk, Inworld.Packets.Routing.FromAgentToPlayer(packet.Routing.Source.Name));
            
            m_Client.m_CurrentConnection.incomingAudioQueue.Enqueue(chunk);
        }
